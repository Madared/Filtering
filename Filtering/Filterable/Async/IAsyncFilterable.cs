...
public interface IAsyncFilterable<T> where T : notnull
{
    ...
    public Task<IFilterable<T>> OrderByDesending<TSelected>(Expression<Func<T, TSelected>> selector);
    public Task<IFilterable<T>> Take(int toTake);
    public Task<IFilterable<T>> Skip(int toSkip);
